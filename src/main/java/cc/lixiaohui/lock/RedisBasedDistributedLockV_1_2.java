 ...
 * <b>可重入实现关键:</b>
 * <ul>
 * <li>在分布式环境中如何确定一个线程? <i><b>mac地址 + jvm pid + threadId</b></i> (mac地址唯一, jvm pid在单机内唯一, threadId在单jvm内唯一)</li>
 * <li>任何一个线程从redis拿到value值后都需要能确定 该锁是否被自己持有, 因此value值要有以下特性: 保存持有锁的主机(mac), jvm pid, 持有锁的线程ID, 重复持有锁的次数</li>
 * </ul>
 * 
 * redis中value设计如下:
 * 
 ...
 *
 */
public class RedisBasedDistributedLockV_1_2 extends AbstractLock {
	...
	private static final Logger logger = LoggerFactory.getLogger(RedisBasedDistributedLockV_1_2.class);
	
	public RedisBasedDistributedLockV_1_2(Jedis jedis, String lockKey, long lockExpires, SocketAddress timeServerAddr) throws IOException {
		this.jedis = jedis;
	...
	protected boolean lock(boolean useTimeout, long time, TimeUnit unit, boolean interrupt) throws InterruptedException{
		...
		// note: 每次进入循环都检查 : 1.是否超时, 若是则return false; 2.是否检查中断(interrupt)被中断, 若需检查中断且被中断, 则抛InterruptedException
		while (useTimeout ? !isTimeout(start, timeout) : true) {
			...
			if (jedis.setnx(lockKey, newLockInfoJson) == 1) { // 条件能成立的唯一情况就是redis中lockKey还未关联value
				// TODO 成功获取到锁, 设置相关标识
				logger.debug("%s get lock(new), lockInfo: %s", Thread.currentThread().getName(), newLockInfoJson);
				return true;
			}
			...
			if (currLockInfo != null && isTimeExpired(currLockInfo.getExpires())) { // lock is expired
				// 锁超时了
				...
				if (isHeldByCurrentThread(currLockInfo)) { // 当前线程持有
					...
					logger.debug("%s get lock(inc), lockInfo: %s", Thread.currentThread().getName(), currLockInfo);
					return true;
				}
			}
		}
	...
	public boolean tryLock() {
		...
		if (jedis.setnx(lockKey, newLockInfo) == 1) { // 条件能成立的唯一情况就是redis中lockKey还未关联value
			// TODO 成功获取到锁, 设置相关标识
			return true;
		}
	...
	protected void unlock0() {
		...
		if (isHeldByCurrentThread(currLockInfo)) {
			if (currLockInfo.getCount() == 1) {
				jedis.del(lockKey);
				logger.debug("%s unlock(del), lockInfo: null", Thread.currentThread().getName());
			} else {
				currLockInfo.decCount(); // 持有锁计数减1
				String json = currLockInfo.toString();
				jedis.set(lockKey, json);
				logger.debug("%s unlock(dec), lockInfo: %s", Thread.currentThread().getName(), json);
			}
		} else {
